...
export const getMarginEngineEvents = async (
  ...
  const marginEngineContract = generateMarginEngineContract(amm.marginEngine, provider);

  if (eventTypes.includes('margin_update')) {
    const eventFilter = marginEngineContract.filters.PositionMarginUpdate();
    const events = await marginEngineContract.queryFilter(eventFilter, fromBlock, toBlock);

    const extendedEvents = events.map((event) => parseMarginUpdateEvent(event, amm, chainId));
