...
export const getInformationPerVAMM = async (
  ...
  switch (information) {
    case 'last_block_active_swaps': {
      const latestBlock = await getActiveSwapsMaxEventBlock(vammAddress, chainId);
      return {
        id: key,
        value: latestBlock || 0,
      };
    ...
    case 'last_block_mints_and_burns': {
      const latestBlock = await getMintsAndBurnsMaxEventBlock(vammAddress, chainId);
      return {
        id: key,
        value: latestBlock || 0,
      };
    ...
    case 'last_tick_pnl': {
      const latestBlock = await getPositionsMaxEventBlock(vammAddress, chainId);
      const latestTick = latestBlock ? await getCurrentTick(chainId, vammAddress, latestBlock) : 0;
...
export const getInformationPerMarginEngine = async (
  ...
  switch (information) {
    case 'last_block_margin_updates': {
      const latestBlock = await getMarginUpdatesMaxEventBlock(marginEngineAddress, chainId);
      return {
        id: key,
        value: latestBlock || 0,
      };
