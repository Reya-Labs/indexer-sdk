...
export const getTableName = (table: Table): string => {
  if (!process.env.ENV) {
    throw new Error('Environment has not been specified in environment variables');
  }

  switch (table) {
    case 'active_swaps': {
      if (!process.env.ACTIVE_SWAPS_TABLE_ID) {
        throw new Error('Active swaps table has not specified in environment variables');
      }
    ...
    case 'mints_and_burns': {
      if (!process.env.MINTS_BURNS_TABLE_ID) {
        throw new Error('Mints and burns table has not specified in environment variables');
      }
    ...
    case 'positions': {
      if (!process.env.POSITIONS_TABLE_ID) {
        throw new Error('Positions table has not specified in environment variables');
      }
