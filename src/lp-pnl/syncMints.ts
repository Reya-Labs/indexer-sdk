...
export const syncMints = async (bigQuery: BigQuery, amms: AMM[], redisClient?: Redis): Promise<void> => {
  ...
  const promises = Object.values(previousMintEvents).map(async ({ events }) => {
    for (const swapEvent of events) {
      ...
      if (redisClient !== undefined) {
        await setFromBlock('mint_lp', swapEvent.chainId, swapEvent.address, swapEvent.blockNumber, redisClient);
      }
    }
  });
